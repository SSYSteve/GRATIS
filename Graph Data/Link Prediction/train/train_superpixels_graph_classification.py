...
"""
    For GCNs
"""
def train_epoch_sparse(model, optimizer, device, data_loader, epoch, args):
    model.train()
    epoch_loss = 0
    epoch_train_acc = 0
    nb_data = 0
...
#        print('###'*10)
#        print('batch_e',batch_graphs.edata['feat'].shape,batch_graphs.edata['feat'][0])
        
        if args.edge_init == 'add':
            batch_graphs.apply_edges(lambda edges: {'feat' : 
                                    edges.data['feat']*(edges.src['feat'] + edges.dst['feat'])/2})
        
        batch_x = batch_graphs.ndata['feat'].to(device)  # num x feat
        batch_e = batch_graphs.edata['feat'].to(device)
        
#        print('batch_x',batch_x.shape)
#        print('batch_e',batch_e.shape,batch_e[0])
#        print('###'*10)
#        sys.exit(-1)
...
def evaluate_network_sparse(model, device, data_loader, epoch, args):
    model.eval()
    epoch_test_loss = 0
    epoch_test_acc = 0
    nb_data = 0
