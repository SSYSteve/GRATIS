...
class BP4D(Dataset):
    def __init__(self, root_path, train=True, fold = 1, transform=None, crop_size = 224, stage=1, loader=default_loader):
        ...
        if self._train:
            ...
            # AU links
            au_relation_list_path = os.path.join(root_path, 'list', 'BP4D_train_co-occ_label_fold' + str(fold) + '.npy')
            au_relation_list = np.load(au_relation_list_path)
            self.data_list = make_dataset(train_image_list, au_relation_list)
            ...
            # AU links
            au_relation_list_path = os.path.join(root_path, 'list', 'BP4D_test_co-occ_label_fold' + str(fold) + '.npy')
            au_relation_list = np.load(au_relation_list_path)
            self.data_list = make_dataset(test_image_list, au_relation_list) 
...
class DISFA(Dataset):
    def __init__(self, root_path, train=True, fold = 1, transform=None, crop_size = 224, stage=1, loader=default_loader):
        ...
        if self._train:
            ...
            train_image_list = open(train_image_list_path).readlines()
            # AU links
            au_relation_list_path = os.path.join(root_path, 'list', 'DISFA_train_co-occ_label_fold' + str(fold) + '.npy')
            au_relation_list = np.load(au_relation_list_path)
            self.data_list = make_dataset(train_image_list, au_relation_list)
            ...
            # AU links
            au_relation_list_path = os.path.join(root_path, 'list', 'DISFA_test_co-occ_label_fold' + str(fold) + '.npy')
            au_relation_list = np.load(au_relation_list_path)
            self.data_list = make_dataset(test_image_list, au_relation_list) 
