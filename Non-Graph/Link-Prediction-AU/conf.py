...
# Network and Loss
parser.add_argument('--arc', default='swin_transformer_base', type=str, choices=['resnet50', 'swin_transformer_base'], help="backbone architecture resnet / swin_transformer")
...
parser.add_argument('--gnn_type', default="GCN", type=str, choices=['GCN', 'GAT'], help="gnn type for co-occurrence detection")
parser.add_argument('--feed_type', default="vertex", type=str, choices=['vertex', 'edge', 'vertex+edge'], help="feeding type to MLP, vertex/edge/vertex+edge")
...
parser.add_argument('--root_dir', type=str, help="directory of the train_link_prediction.py")
...
def get_config():
    ...
    cfg = parser2dict()
    if cfg.dataset == 'BP4D':
        with open(cfg.root_dir + 'config/BP4D_config.yaml', 'r') as f:
            datasets_cfg = yaml.load(f)
            datasets_cfg = edict(datasets_cfg)

    elif cfg.dataset == 'DISFA':
        with open(cfg.root_dir + 'config/DISFA_config.yaml', 'r') as f:
            datasets_cfg = yaml.load(f)
            datasets_cfg = edict(datasets_cfg)
...
def set_outdir(conf):
    ...
    ensure_dir(outdir)
    conf['outdir'] = outdir
    shutil.copyfile(conf.root_dir + "model/MEFL.py", os.path.join(outdir,'MEFL.py'))
    shutil.copyfile(conf.root_dir + "model/ANFL.py", os.path.join(outdir,'ANFL.py'))
