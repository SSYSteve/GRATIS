...
- After obtaining all of `DISFA_test_label_fold*.txt`, `DISFA_train_label_fold*.txt`, `BP4D_test_label_fold*.txt`, `BP4D_train_label_fold*.txt` files, the labels for link prediction task can be obtained through utilizing:

```
python label_link_generator.py
```
