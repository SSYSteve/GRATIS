...
# Datasets
parser.add_argument('--dataset', default="FER2013", type=str, help="experiment dataset FER2013")
...
def get_config():
    ...
    if cfg.dataset == 'FER2013':
        with open('/content/drive/My Drive/MEFR/config/FER2013_config.yaml', 'r') as f:
...
def set_outdir(conf):
    default_outdir = '/content/drive/My Drive/MEFR/results'
    if 'timedir' in conf:
    ...
    ensure_dir(outdir)
    conf['outdir'] = outdir
    shutil.copyfile("/content/drive/My Drive/MEFR/model/MEFL.py", os.path.join(outdir,'MEFL.py'))
    shutil.copyfile("/content/drive/My Drive/MEFR/model/ANFL.py", os.path.join(outdir,'ANFL.py'))
