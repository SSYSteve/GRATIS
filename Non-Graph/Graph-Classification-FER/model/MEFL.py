...
class Head(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        self.sc = nn.Parameter(torch.FloatTensor(torch.zeros(7, self.in_channels)))
    ...
    def forward(self, x):
        ...
        # MEFL
        f_e = torch.zeros(f_v.shape[0], 16, 512).cpu().detach().numpy()
        ...
        b, n, c = f_v.shape
        f_v = torch.flatten(f_v, start_dim=1)
        cl =  self.fc(f_v)
        cl = cl.view(b, 7, c)
        sc = self.sc
        sc = self.relu(sc)  
        sc = F.normalize(sc, p=2, dim=-1)
        cl = F.normalize(cl, p=2, dim=-1)
        cl = (cl * sc.view(1, 7, c)).sum(dim=-1, keepdim=False)
